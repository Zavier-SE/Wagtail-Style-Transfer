...
class ImagePage(Page):
    ...
    def serve(self, request):
        ...
        #     context["my_result_file_names"].append(str(f'{str(filename)}'))
            ...
            # a = urlparse(URL)
            # ## manually download the photo into DeOldify folder
            # #before_file = os.path.basename(a.path)
            # # before_file = 'maria-parents-01.jpg' # 'lun-parents-01.jpg' #  'lady-bw.jpg'   # 'DSC_0001.jpg'   # 'rato-01.jpg'
            # print(files[0])
            # before_file = files[0]
            ...
            # after_image_cv2 = cv2.cvtColor(np.array(after_image),cv2.COLOR_RGB2BGR)
            # cv2.imwrite('media/Result/colorized-local.jpg', after_image_cv2)
            # context["my_result_file_names"].append('colorized-local.jpg')
            # plt.imshow(after_image_cv2)
            ### end own code
